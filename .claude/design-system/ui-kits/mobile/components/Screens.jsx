...
const CATEGORIES = ['전체', '축구', '야구', '농구', '배구', '럭비', 'e스포츠'];
...
function HomeScreen({ liked, onLike, onSelect, dark }) {
  ...
  return (
    <>
      ...
      <div style={{ padding: '14px 16px 8px' }}>
        <div style={{
          ...
          <div style={{ position:'relative' }}>
            <div style={{ fontFamily: "'IAMAPLAYER','Bebas Neue',sans-serif", fontSize: 13, letterSpacing: '0.1em', color: '#FF2E4D' }}>WEEKLY DRAFT</div>
            <div style={{ fontFamily: 'Pretendard', fontWeight: 800, fontSize: 22, lineHeight: 1.2, marginTop: 4, letterSpacing: '-0.02em' }}>이번 주 가장 빠르게<br/>거래된 유니폼</div>
      ...
      <div style={{ display: 'flex', gap: 8, padding: '4px 16px 12px', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        {CATEGORIES.map(c => (
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '8px 16px 12px' }}>
        <h2 style={{ fontFamily: 'Pretendard', fontWeight: 700, fontSize: 18, margin: 0, letterSpacing: '-0.015em' }}>오늘의 유니폼</h2>
        <button style={{ background: 'none', border: 'none', color: '#5A6A7A', fontSize: 12, fontFamily: 'Pretendard', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          <I.filter size={14}/> 필터
        </button>
      </div>
  ...
  return (
    <>
      ...
      <div style={{ padding: '14px 16px 8px', fontFamily: 'Pretendard' }}>
        ...
        <h2 style={{ fontFamily: 'Pretendard', fontWeight: 800, fontSize: 22, margin: '10px 0 4px', letterSpacing: '-0.02em', color: '#0D1B2A' }} data-text-main>
          {item.title}
        </h2>
        ...
        <div style={{ marginTop: 14, display: 'flex', alignItems: 'baseline', gap: 10 }}>
          <span style={{ fontFamily: "'IAMAPLAYER','Bebas Neue',sans-serif", fontSize: 36, color: '#002147', letterSpacing: '0.02em' }} data-price>
...
function CommunityScreen({ dark }) {
  ...
  const posts = [
    { id:'p1', tag:'이적시장', title:'손흥민 폼 회복했네요. 이번 시즌 기대됩니다', author:'spurs_fan', time:'12분 전', likes:64, chats:18, comments: '23', hot:true },
    { id:'p2', tag:'구단', title:'KIA 우승 기념 유니폼 어디서 구하나요?', author:'tigerking', time:'1시간 전', likes:21, chats:7,  comments: '7' },
    { id:'p3', tag:'리뷰', title:'어센틱 vs 레플리카 솔직 비교', author:'jersey_lab', time:'3시간 전', likes:155, chats:42, comments: '42', hot:true },
    { id:'p4', tag:'인기', title:'중고 거래 사이즈 안 맞을 때 환불 가능?', author:'newbie22', time:'어제', likes:18, chats:11,  comments: '11' },
  ];
  return (
    <>
      <GNB dark={dark} title="커뮤니티" right={<><IconBtn><I.search/></IconBtn><IconBtn><I.bell/></IconBtn></>}/>
      <div style={{ display: 'flex', gap: 4, padding: '10px 12px 4px', borderBottom: '1px solid rgba(13,27,42,.10)' }} data-divider>
        {tabs.map(x => (
      ...
      <div>
        {posts.map(p => (
          <div key={p.id} style={{ padding: '14px 16px', borderBottom: '1px solid rgba(13,27,42,.08)', fontFamily: 'Pretendard', cursor: 'pointer' }} data-divider>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              <Badge kind="soft">{p.tag}</Badge>
              {p.hot ? <Badge kind="accent" sport>HOT</Badge> : null}
            </div>
            <h3 style={{ fontFamily: 'Pretendard', fontWeight: 700, fontSize: 15, margin: 0, color: '#0D1B2A', lineHeight: 1.4, letterSpacing:'-0.01em' }}>
              {p.title}
            </h3>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8, color: '#5A6A7A', fontSize: 12 }}>
              ...
              <span style={{ marginLeft:'auto', display: 'inline-flex', gap: 10, color:'#9BAAB9' }}>
                <span style={{ display:'inline-flex', alignItems:'center', gap:3 }}><I.heart size={12}/>{p.likes}</span>
                <span style={{ display:'inline-flex', alignItems:'center', gap:3 }}><I.msg size={12}/>{p.comments}</span>
              </span>
            </div>
          </div>
        ))}
        ...
        {chats.map(c => (
          <div key={c.id} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 13, color: '#5A6A7A', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', marginTop:2 }}>{c.last}</div>
            </div>
            {c.unread ? (
              <span style={{
  ...
  return (
    <>
      ...
      <div style={{ padding: '20px 16px', display: 'flex', alignItems: 'center', gap: 14, fontFamily: 'Pretendard' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'IAMAPLAYER','Bebas Neue',sans-serif", fontSize: 22, letterSpacing: '0.04em' }}>JERSEY_07</div>
          <div style={{ fontSize: 12, color: '#5A6A7A', marginTop: 2, display:'inline-flex', alignItems:'center', gap:4 }}>
            <I.star size={12} fill/> 4.9 · 거래 23회
          </div>
        </div>
        <Button kind="ghost" size="sm">프로필 편집</Button>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', borderTop: '1px solid rgba(13,27,42,.08)', borderBottom: '1px solid rgba(13,27,42,.08)' }} data-divider>
      ...
      {[
        ...
        <div key={x} style={{ padding: '14px 16px', borderBottom: '1px solid rgba(13,27,42,.06)', fontFamily: 'Pretendard', fontSize: 14, color: '#0D1B2A', cursor:'pointer', display:'flex', alignItems:'center' }}>
          {x}
