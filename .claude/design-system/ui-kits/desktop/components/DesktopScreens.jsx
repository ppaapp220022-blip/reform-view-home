...
function FilterRail({dark}) {
  ...
  return (
    <aside data-surface data-card style={{
      ...
      <Section title="CATEGORY">
        {['야구', '축구', '농구', '배구', '럭비', 'e스포츠'].map(c => (
          <label key={c} style={{
            ...
            <input type="radio" checked={cat === c} onChange={() => setCat(c)}
      ...
      <Section title="PRICE">
        <div style={{fontFamily: "'IAMAPLAYER','Bebas Neue',sans-serif", fontSize: 18, color: fg}}
             data-text-main>
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'absolute',
        inset: 0,
        backgroundImage: 'repeating-linear-gradient(115deg, rgba(255,255,255,.06) 0 2px, transparent 2px 18px)'
      }}/>
    ...
    <div style={{maxWidth: 1280, margin: '0 auto', padding: '24px 28px 64px', fontFamily: 'Pretendard'}}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 460px', gap: 32}}>
        ...
        <div>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginTop: 10}}>
            {['navy', 'red', 'dark', 'pitch'].map((g, i) => (
              <div key={g} data-card style={{
                borderRadius: 8,
                overflow: 'hidden',
                border: `1px solid ${i === 0 ? '#002147' : border}`,
                cursor: 'pointer'
              }}>
        ...
        <div>
          ...
          <div style={{display: 'flex', alignItems: 'center', gap: 6, color: sub, fontSize: 13}}
               data-text-sub>
          ...
          <div style={{
            ...
            <div style={{display: 'flex', alignItems: 'center', gap: 12}}>
              <div style={{
                ...
                height: 44,
                borderRadius: 99,
                background: 'linear-gradient(135deg,#FF2E4D,#002147)'
              }}/>
              <div style={{flex: 1}}>
                <div style={{fontWeight: 700, fontSize: 14, color: fg}}
                     data-text-main>jersey_collector
                </div>
                <div style={{
                  display: 'inline-flex',
                  gap: 4,
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 16}}>
            {[
              ...
              <div key={k} data-surface-sunken
                   style={{background: '#F4F6F9', borderRadius: 8, padding: '10px 12px'}}>
                <div style={{fontSize: 11, color: '#5A6A7A'}}>{k}</div>
                <div style={{fontSize: 14, fontWeight: 600, color: '#0D1B2A', marginTop: 2}}
                     data-text-main>{v}</div>
              </div>
            ))}
          </div>

          <div style={{display: 'flex', gap: 8, marginTop: 22}}>
            <Button kind="ghost" size="lg" onClick={() => onLike(item.id)}
                    style={{flex: '0 0 56px', display: 'grid', placeItems: 'center'}}>
      ...
      <div style={{marginTop: 36, display: 'grid', gridTemplateColumns: '1fr 320px', gap: 32}}>
        <div>
          <h2 style={{fontWeight: 800, fontSize: 18, margin: '0 0 12px', letterSpacing: '-0.015em'}}
              data-text-main>상품 설명</h2>
        ...
        <div data-card data-surface
             style={{borderRadius: 12, border: `1px solid ${border}`, padding: 18, height: 'fit-content'}}>
          <div style={{
            fontFamily: "'IAMAPLAYER','Bebas Neue',sans-serif",
            fontSize: 13,
            letterSpacing: '0.1em',
...
function CommunityScreenD({dark}) {
  ...
  const posts = [
    ...
    },
    {
      id: 'p6',
      tag: '구단',
      title: '두산 베어스 23시즌 유니폼 실측 사이즈 정리',
      author: 'bears_archive',
      time: '2일 전',
      likes: 74,
      comments: 18,
      views: '1.8k'
  ...
  return (
    <div style={{maxWidth: 1280, margin: '0 auto', padding: '24px 28px 64px', fontFamily: 'Pretendard'}}>
      <div style={{display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 20}}>
        <h1 style={{fontWeight: 800, fontSize: 28, margin: 0, letterSpacing: '-0.02em'}}
            data-text-main>커뮤니티</h1>
        <span style={{
          fontFamily: "'IAMAPLAYER','Bebas Neue',sans-serif",
          fontSize: 13,
          letterSpacing: '0.1em',
          color: sub
        }}>FANS · TRANSFERS · REVIEWS</span>
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24}}>
            ...
            {posts.map(p => (
              <tr key={p.id} style={{cursor: 'pointer'}}>
                <td style={{padding: '14px 8px', borderBottom: `1px solid ${border}`}} data-divider>
                  <Badge kind="soft">{p.tag}</Badge></td>
                <td style={{padding: '14px 8px', borderBottom: `1px solid ${border}`}} data-divider>
                  <div style={{display: 'flex', alignItems: 'center', gap: 8}}>
                                        <span style={{fontWeight: 600, fontSize: 14, color: fg}}
                                              data-text-main>{p.title}</span>
        ...
        <aside style={{display: 'flex', flexDirection: 'column', gap: 16}}>
          ...
          <div data-card style={{
            borderRadius: 12,
            padding: 20,
            background: 'linear-gradient(135deg,#002147,#1A3051)',
            color: '#fff',
            position: 'relative',
            overflow: 'hidden'
          }}>
            ...
            <div style={{position: 'relative'}}>
              ...
              <div style={{fontWeight: 800, fontSize: 18, marginTop: 6, lineHeight: 1.3}}>오늘의 베스트<br/>거래
                후기
              </div>
