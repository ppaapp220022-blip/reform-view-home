 ...
 *   FormSection      — 제목/팀/리그/사이즈/등급/거래방식/가격/설명 입력
 *   AiPanel          — AI 설명 추천 + 위험도 탐지 (오른쪽 사이드패널)
 *   LivePreview      — 작성 중 카드 프리뷰
 *
 * AI 기능: 목 시뮬레이션 (백엔드 미연동)
 *   - 설명 추천: 제목/팀/리그/등급 입력 후 버튼 클릭 시 AI 추천 텍스트 생성
...
/** AI 설명 추천 템플릿 (목) */
function generateAiDescription(title: string, team: string, league: string, grade: Grade): string {
...
function ImageUploader({
  ...
  /* 파일 선택 시 색상 플레이스홀더로 시뮬레이션 */
  const fileRef = useRef<HTMLInputElement>(null)
  const mockColors = ['#B5222B','#1A3051','#034694','#1A7A40','#A50044','#6B0078','#C8102E','#003087']

  function handleFile() {
    if (images.length >= MAX) return
    onAdd(mockColors[images.length % mockColors.length])
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 flex-wrap">
        {images.map((c, i) => (
          <div key={i} className="relative rounded-xl overflow-hidden flex-shrink-0" style={{ width:80, height:80, background:c }}>
            <div className="absolute inset-0" style={{ backgroundImage:'repeating-linear-gradient(115deg, rgba(255,255,255,.08) 0 2px, transparent 2px 10px)' }} />
            {i === 0 && (
              <span className="absolute bottom-1 left-1 text-[9px] font-bold px-1 py-0.5 rounded" style={{ background:'rgba(0,0,0,.45)', color:'#fff' }}>대표</span>
            )}
            <button
              onClick={() => onRemove(i)}
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <ChevronDown size={15} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" color="var(--color-text-hint)" />
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div className="relative flex items-center">
        ...
        />
        {suffix && (
          <span className="absolute right-3 text-sm pointer-events-none" style={{ color:'var(--color-text-hint)' }}>{suffix}</span>
        )}
      </div>
    </div>
  )
...
function RiskBanner({ level, msg }: { level: RiskLevel; msg: string }) {
  ...
  return (
    <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl mt-2" style={{ background:c.bg, border:`1px solid ${c.text}33` }}>
...
function AiPanel({
  ...
  async function requestAiDescription() {
    ...
    setAiResult(generateAiDescription(form.title, form.team, form.league, form.grade))
    setAiLoading(false)
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* 위험 탐지 */}
      <div className="rounded-2xl overflow-hidden" style={{ border:'1px solid var(--color-border)' }}>
        <div className="flex items-center gap-2 px-4 py-3" style={{ background: risks.length > 0 ? 'rgba(255,46,77,.06)' : 'var(--color-surface-raised)', borderBottom:'1px solid var(--color-border)' }}>
          {risks.length > 0
            ? <AlertTriangle size={15} color="var(--color-accent)" />
            : <CheckCircle2 size={15} color="var(--color-success)" />
          }
          <span className="text-sm font-bold" style={{ color: risks.length > 0 ? 'var(--color-accent)' : 'var(--color-success)' }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border:'1px solid var(--color-border)' }}>
        ...
        <div className="p-4" style={{ background:'var(--color-surface)' }}>
          {/* 미니 카드 */}
          <div className="rounded-xl overflow-hidden" style={{ border:'1px solid var(--color-border)', maxWidth:200, margin:'0 auto' }}>
            <div
              className="relative"
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-6">
            ...
            <div className="rounded-2xl p-5" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)' }}>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <FormSelect
                    label="리그"
                    value={form.league} onChange={v => update('league', v)}
                    options={leagueOptions} placeholder="리그 선택"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <FormInput
            ...
            <div className="rounded-2xl p-5" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)' }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs" style={{ color:'var(--color-text-hint)' }}>{form.description.length}/1000</span>
              </div>
              <textarea
