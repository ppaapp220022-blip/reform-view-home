...
import {
  ChevronLeft, AlertTriangle, Ban, UserX, ShieldCheck,
  Mail, Calendar, Star, ShoppingBag, Flag, ChevronRight,
} from 'lucide-react'
...
const ACTION_CONFIG: Record<AdminAction, {
  ...
  warn:      { label: '경고',      confirmLabel: '경고 조치',   color: 'var(--color-warning)', desc: '회원에게 경고 1회를 부여합니다. 누적 3회 시 자동 정지됩니다.' },
  suspend:   { label: '정지',      confirmLabel: '계정 정지',   color: 'var(--color-accent)',  desc: '계정을 즉시 정지합니다. 로그인 및 거래가 차단됩니다.' },
  unsuspend: { label: '정지 해제', confirmLabel: '정지 해제',   color: 'var(--color-success)', desc: '계정 정지를 해제하고 정상 이용 상태로 복구합니다.' },
  withdraw:  { label: '강제 탈퇴', confirmLabel: '강제 탈퇴 실행', color: 'var(--color-accent)', desc: '회원을 강제 탈퇴 처리합니다. 이 작업은 되돌릴 수 없습니다.' },
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      >
        ...
        <p className="text-[13px] text-center mb-6 leading-relaxed" style={{ color: 'var(--color-text-sub)' }}>
          {cfg.desc}
        </p>
...
export default function AdminMemberDetailPage() {
  ...
  function confirmAction() {
    ...
    if (pendingAction === 'warn')      { updated.warningCount += 1 }
    if (pendingAction === 'suspend')   { updated.status = 'SUSPENDED' }
    if (pendingAction === 'unsuspend') { updated.status = 'ACTIVE' }
    if (pendingAction === 'withdraw')  { updated.status = 'WITHDRAWN' }
    setMember(updated)
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_280px] gap-6">
          ...
          >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px]" style={{ color: 'var(--color-text-sub)' }}>{member.email}</span>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px]" style={{ color: 'var(--color-text-sub)', fontFamily: "'IAMAPLAYER',Giants,sans-serif" }}>
            ...
            <div className="grid grid-cols-3 gap-3 mt-5 pt-5"
              ...
              {[
                ...
                <div key={s.label} className="text-center">
                  ...
                  <p className="text-[11px]" style={{ color: 'var(--color-text-hint)' }}>{s.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 pt-4" style={{ borderTop: '1px solid var(--color-border)' }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[12px] font-medium" style={{ color: 'var(--color-text-sub)' }}>
                  누적 경고 횟수
                </span>
                <span
            ...
            <div className="flex flex-col gap-0">
              {MOCK_TRADES.map((t) => {
                ...
                return (
                  ...
                  >
                    <span
                      className="w-12 text-center text-[10px] font-medium px-1.5 py-0.5 rounded-full flex-shrink-0"
                      style={{
                    ...
                    <p className="flex-1 text-[13px] truncate" style={{ color: 'var(--color-text-main)' }}>
                      {t.title}
                    </p>
                    <span
              ...
              {MOCK_REPORTS.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-medium" style={{ color: 'var(--color-text-main)' }}>
                        {r.reason}
                      </p>
                      <p className="text-[12px]" style={{ color: 'var(--color-text-hint)' }}>
                        {r.detail}
                    ...
                    <Link
                      ...
                      className="flex-shrink-0 text-[11px] font-medium flex items-center gap-0.5 transition-colors
        ...
        <div className="flex flex-col gap-4">
          <div
            ...
            style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}
          >
            <p className="text-[13px] font-bold mb-4" style={{ color: 'var(--color-text-main)' }}>
              관리자 조치
            </p>
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <button
                ...
                className="w-full h-[44px] rounded-[10px] flex items-center gap-2.5 px-4 text-[13px] font-semibold
              ...
              <div className="pt-2" style={{ borderTop: '1px solid var(--color-border)' }}>
                ...
                <p className="text-[10px] mt-1.5 text-center" style={{ color: 'var(--color-text-hint)' }}>
                  이 작업은 되돌릴 수 없습니다
                </p>
              </div>
            </div>
